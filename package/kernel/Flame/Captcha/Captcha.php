...
class Captcha
{
    ...
    //缓存Key
    protected string $captchaName = 'sys_captcha_';
    ...
    protected function generate(string $uuid): array
    {
        ...
        Cache::set($this->captchaName.md5($uuid), [
            'key' => $key,
        ], 600);
    ...
    {
        $name = $this->captchaName.md5($uuid);

        if (! Cache::has($name)) {
            return false;
        }

        $captcha = Cache::get($name);
